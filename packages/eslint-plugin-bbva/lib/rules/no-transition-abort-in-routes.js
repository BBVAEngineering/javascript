...
function checkAbortTransition(context, transitionArgument) {
  ...
  const transitionVariable = context
    .getScope()
    .set.get(transitionArgument.name);
  const transitionAbortCall = transitionVariable.references.find(
    (ref) =>
  ...
  if (transitionAbortCall) {
    context.report({
      ...
      message:
        "avoid aborting the transition on route hooks, use an abort with fallback method instead",
    });
  }
}
...
module.exports = {
  meta: {
    ...
    docs: {
      description:
        "`transition.abort` can lead to navigation problems if executed on route hooks while the page refreshes",
      recommended: false,
    },
    fixable: null,
    schema: [],
  ...
  create(context) {
    if (mightBeEmberRouteFile(context.getFilename())) {
      return {
        "Property[key.name=beforeModel] > FunctionExpression[params.length=1]"(
          node
        ) {
        ...
        "Property[key.name=afterModel] > FunctionExpression[params.length=2]"(
          node
        ) {
          checkAbortTransition(context, node.params[1]);
        },
        "Property[key.name=redirect] > FunctionExpression[params.length=2]"(
          node
        ) {
          checkAbortTransition(context, node.params[1]);
        },
